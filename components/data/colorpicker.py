...
from PySide6.QtCore import Qt, Signal, QPropertyAnimation, QRect, Property, QByteArray, QPoint, QParallelAnimationGroup
...
class FluentColorButton(QPushButton):
    ...
    def _setup_animations(self):
        ...
        # Create hover animation with enhanced easing
    ...
    def enterEvent(self, event):
        ...
        if not self._is_selected:  # Only animate if not selected
    ...
    def paintEvent(self, _event):
        """Paint the color button with enhanced effects"""
        painter = QPainter(self)
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)

        # Calculate scaled rect with smooth transformation
        rect = self.rect()
...
class FluentColorWheel(QWidget):
    ...
    def getColor(self) -> QColor:
        ...
        color = QColor()
        color.setHsvF(self._hue, self._saturation, self._value,
                      1.0)  # Alpha is 1.0 by default
        return color
    ...
    def paintEvent(self, _event):
        ...
        for angle_deg in range(360):
            ...
            painter.drawLine(QPoint(int(x1), int(y1)),
                             QPoint(int(x2), int(y2)))

        # Draw current hue indicator
        # Hue is [0,1), angle is [0, 2pi)
        current_angle_rad = self._hue * 2 * math.pi
    ...
    def _draw_sv_triangle(self, painter: QPainter, center: QPoint, max_radius: int):
        ...
        sv_rect = QRect(int(sv_rect_top_left_x), int(
            sv_rect_top_left_y), int(sv_rect_size), int(sv_rect_size))

        # Draw saturation gradient (horizontal: white to hue_color)
        for i in range(int(sv_rect.width())):
            s = i / sv_rect.width()
            grad_color = QColor()
            # Use current value for this sweep
            grad_color.setHsvF(self._hue, s, self._value)
            painter.setPen(QPen(grad_color))
            painter.drawLine(sv_rect.left() + i, sv_rect.top(),
                             sv_rect.left() + i, sv_rect.bottom())
        ...
        indicator_sv_y = sv_rect.top() + (1.0 - self._value) * \
            sv_rect.height()  # V=1 is top, V=0 is bottom

        painter.setPen(QPen(QColor(Qt.GlobalColor.black)
                       if self._value > 0.5 else QColor(Qt.GlobalColor.white), 1))
        painter.setBrush(Qt.BrushStyle.NoBrush)
        painter.drawEllipse(
            QPoint(int(indicator_sv_x), int(indicator_sv_y)), 3, 3)
    ...
    def _update_color_from_position(self, pos: QPoint):
        ...
        if inner_radius <= distance <= outer_radius:
            ...
            if sv_rect.contains(pos):
                ...
                # V is inverted
                new_v = 1.0 - min(max(relative_y / sv_rect.height(), 0.0), 1.0)
...
class FluentColorPicker(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self._predefined_colors: List[QColor] = [
            QColor("#FF6B6B"), QColor("#4ECDC4"), QColor(
                "#45B7D1"), QColor("#96CEB4"),
            QColor("#FECA57"), QColor("#FF9FF3"), QColor(
                "#54A0FF"), QColor("#5F27CD"),
            QColor("#00D2D3"), QColor("#FF9F43"), QColor(
                "#10AC84"), QColor("#EE5A6F"),
            QColor("#0078d4"), QColor("#005a9e"), QColor(
                "#004578"), QColor("#002c4a"),
            QColor("#6264A7"), QColor("#8764B8"), QColor(
                "#8E8CD8"), QColor("#B4A7D6"),
            QColor("#D13438"), QColor("#FF4B4B"), QColor(
                "#FF6B6B"), QColor("#FF9999")
        ]
    ...
    def _setup_current_color(self, layout: QVBoxLayout):
        ...
        self._color_display.setFrameStyle(
            QFrame.Shape.Box | QFrame.Shadow.Sunken)  # Example style
    ...
    def _setup_color_wheel(self, layout: QVBoxLayout):
        ...
        self._color_wheel.colorChanged.connect(
            self._on_wheel_or_slider_changed)  # Connect to unified handler
    ...
    def _setup_rgb_sliders(self, layout: QVBoxLayout):
        ...
        for i, component in enumerate(['R', 'G', 'B']):
            ...
            slider.valueChanged.connect(
                self._on_rgb_slider_changed)  # Specific handler
    ...
    def _update_color_display(self):
        if hasattr(self, '_color_display'):
            self._color_display.setStyleSheet(
                f"background-color: {self._current_color.name()}; border: 1px solid #ccc; border-radius: 4px;")
    ...
    def _on_predefined_color_selected(self, color: QColor):
        """Handle color selection with enhanced animation"""
        # Create a smooth transition animation
        prev_color = self.getColor()
        self.setColor(color)
    ...
    def _on_hex_changed(self, text: str):
        ...
        if text.startswith('#') and (len(text) == 7 or len(text) == 9):
