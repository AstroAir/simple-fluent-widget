...
class FluentCalendar(QWidget):
    ...
    def update_calendar(self):
        ...
        self.month_year_label.setText(
            f"{self._current_year}年 {month_names[self._current_month - 1]}")
        ...
        for row in range(6):
            for col in range(7):
                ...
                if cell_index < start_day or day > days_in_month:
                    ...
                    current_date = QDate(
                        self._current_year, self._current_month, day)
    ...
    def on_day_clicked(self):
        ...
        if isinstance(sender, CalendarDayButton) and sender.day() > 0:
            selected_date = QDate(self._current_year,
                                  self._current_month, sender.day())
...
class FluentDatePicker(QWidget):
    ...
    def show_calendar(self):
        ...
        if self.calendar_popup is None:
            ...
            self.calendar_popup.calendar.dateSelected.connect(
...
class FluentTimePicker(QWidget):
    ...
    def on_time_changed(self):
        ...
        if not self._use_24_hour:
            ...
            if hour == 12:
                hour = 0 if not is_pm else 12  # Corrected AM/PM logic for 12
            # Ensure hour is not already PM (e.g. 13-23)
    ...
    def setShowSeconds(self, show_seconds: bool):
        ...
        self.update_format()
        self.update_controls()  # Also update controls when seconds visibility changes
...
class FluentDateTimePicker(QWidget):
    ...
    def __init__(self, parent: Optional[QWidget] = None):
        ...
        self.setup_ui()
        # Theme is applied by child components, so no direct apply_theme here
        # However, connecting to theme_manager might be useful if this widget itself had styles
        # theme_manager.theme_changed.connect(self.apply_theme) # Not strictly needed if children handle it
    ...
    def setDateTime(self, datetime: QDateTime):
        ...
        if datetime.isValid():
            ...
            # No direct emit here, changes in children will trigger signals
    ...
    def setDate(self, date: QDate):
        ...
        if date.isValid():
            current_time = self._datetime.time()
            self._datetime = QDateTime(date, current_time)
            # This will emit dateChanged from date_picker
            self.date_picker.setDate(date)
            # Also emit combined change
            self.dateTimeChanged.emit(self._datetime)
    ...
    def setTime(self, time: QTime):
        ...
        if time.isValid():
            current_date = self._datetime.date()
            self._datetime = QDateTime(current_date, time)
            # This will emit timeChanged from time_picker
    ...
    # def apply_theme(self): # Not needed if children handle their themes
    #     """Apply current theme"""
