...
class NavigationSection:
    """Configuration for navigation sections"""
    id: str
    title: str
...
class FluentSidebar(FluentCompositeWidget, SearchableProtocol):
    ...
    def __init__(self,
        ...
        self._state = NavigationState.NORMAL
    ...
    def _setup_navigation_area(self):
        ...
        self.nav_scroll.setSizePolicy(QSizePolicy.Policy.Preferred, QSizePolicy.Policy.Expanding)
    ...
    def add_item_from_config(self, section_id: str, config: NavigationItem) -> FluentStandardButton:
        ...
        if config.callback:
            button.clicked.connect(
                lambda checked=False, cfg=config: cfg.callback(cfg.id) if cfg.callback else None
            )
    ...
    def add_navigation_section(self, section_id: str, title: str,
        ...
        if collapsible:
            header_button.clicked.connect(partial(self._toggle_section, section_id))
    ...
    def _toggle_collapse(self):
        ...
        new_state = NavigationState.COLLAPSED if self._state == NavigationState.NORMAL else NavigationState.NORMAL
    ...
    def set_state(self, state: NavigationState) -> None:
        ...
        if state == NavigationState.COLLAPSED:
            target_width = self._collapsed_width
        elif state == NavigationState.NORMAL:
            target_width = self._width
        elif state == NavigationState.HIDDEN:
            target_width = 0
        else:
            target_width = self._width
    ...
    def _animate_width_change(self, target_width: int) -> None:
        ...
        if not hasattr(self, "_width_anim_group"):
            self._min_width_anim = QPropertyAnimation(self, QByteArray(b"minimumWidth"))
            self._min_width_anim.setDuration(300)
            self._min_width_anim.setEasingCurve(QEasingCurve.Type.OutCubic)

            self._max_width_anim = QPropertyAnimation(self, QByteArray(b"maximumWidth"))
    ...
    def _update_content_visibility(self, state: NavigationState) -> None:
        """Update content visibility based on state"""
        is_collapsed = state == NavigationState.COLLAPSED
        is_hidden = state == NavigationState.HIDDEN
            ...
            for item in section_data['items']:
                ...
                if is_collapsed:
                    ...
                    if not icon.isNull():
                        ...
                        button.setText(original_text[0] if original_text else "")
    ...
    def _update_section_visibility(self) -> None:
        ...
        for section_id, section_data in self._sections.items():
            has_visible_items = any(item['visible'] for item in section_data['items'])
    ...
    def update_item_badge(self, item_id: str, badge_text: str) -> bool:
        ...
        if self._state != NavigationState.COLLAPSED:
            if badge_text:
...
class FluentHeaderNavigation(FluentCompositeWidget):
    ...
    def _setup_header_ui(self):
        ...
        self._main_layout = FluentLayoutBuilder.create_horizontal_layout(spacing=16)
        self.setLayout(self._main_layout)
    ...
    def _setup_left_section(self):
        ...
        if self._config.show_breadcrumbs:
            ...
            self.breadcrumb_layout = FluentLayoutBuilder.create_horizontal_layout(spacing=4)
    ...
    def _setup_right_section(self):
        ...
        self.actions_layout = FluentLayoutBuilder.create_horizontal_layout(spacing=4)
        self.actions_widget.setLayout(self.actions_layout)
        right_layout.addWidget(self.actions_widget)

        if self._config.show_profile:
            self.profile_button = FluentStandardButton(self._config.profile_text, size=(40, 40))
    ...
    def add_action_from_config(self, config: HeaderAction) -> FluentStandardButton:
        ...
        if config.callback:
            action_button.clicked.connect(lambda checked=False, cfg=config: cfg.callback(cfg.id) if cfg.callback else None)
        else:
            action_button.clicked.connect(lambda: self.action_triggered.emit(config.id))
    ...
    def add_action(self, action_id: str, title: str,
        ...
        action_config = HeaderAction(id=action_id, title=title, tooltip=tooltip)
    ...
    def set_breadcrumbs(self, breadcrumbs: List[str]):
        ...
        for i, crumb in enumerate(breadcrumbs):
            if i > 0:
                ...
                self.breadcrumb_layout.addWidget(separator)            crumb_button = FluentStandardButton(crumb, size=(None, 24))
...
class FluentBreadcrumbBar(FluentCompositeWidget):
    ...
    def set_breadcrumbs(self, breadcrumbs: List[str], animate: Optional[bool] = None):
        ...
        for i, crumb in enumerate(display_breadcrumbs):
            ...
            button = self._create_breadcrumb_button(crumb, i, len(display_breadcrumbs))
    ...
    def _create_breadcrumb_button(self, text: str, index: int, total: int) -> FluentStandardButton:
        ...
        button.clicked.connect(lambda: self._on_breadcrumb_clicked(actual_index))
    ...
    def _calculate_actual_index(self, display_index: int, total_display: int) -> int:
        ...
        if len(self._breadcrumbs) > self._config.max_visible_items:
            ...
            if display_index < ellipsis_pos:
                ...
                # After ellipsis
                offset = len(self._breadcrumbs) - total_display + ellipsis_pos + 1
    ...
    def _execute_parallel_animations(self, animations: List[QPropertyAnimation]):
        ...
        self._animation_group.start(QAbstractAnimation.DeletionPolicy.DeleteWhenStopped)
    ...
    def set_from_path(self, path: str, separator: str = "/"):
        ...
        breadcrumbs = [part.strip() for part in path.split(separator) if part.strip()]
