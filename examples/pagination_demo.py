...
class PaginationDemo(QMainWindow):
    ...
    def __init__(self):
        ...
        tab_widget.addTab(
            self._create_basic_pagination_demo(), "Basic Pagination")
        tab_widget.addTab(self._create_modes_demo(), "Display Modes")
        tab_widget.addTab(
            self._create_simple_pagination_demo(), "Simple Pagination")
    ...
    def _create_basic_pagination_demo(self) -> QWidget:
        ...
        layout_grid.addWidget(
            QLabel("Small Dataset (50 items, 10 per page):"), 0, 0)
        ...
        # Medium dataset
        layout_grid.addWidget(
            QLabel("Medium Dataset (500 items, 20 per page):"), 0, 1)
        ...
        # Large dataset
        layout_grid.addWidget(
            QLabel("Large Dataset (10000 items, 50 per page):"), 2, 0)
        ...
        custom_options_info = QLabel(
            "Custom page size options: [5, 15, 30, 60, 100]")
    ...
    def _create_modes_demo(self) -> QWidget:
        ...
        full_mode_info = QLabel(
            "Shows all controls: page buttons, page size selector, jumper, total count")
        ...
        compact_mode_info = QLabel(
            "Shows essential controls, hides some elements when only one page")
        ...
        simple_mode_info = QLabel(
            "Shows only previous/next buttons and total count")
        ...
        full_mode_btn.clicked.connect(
            lambda: self._set_dynamic_mode(FluentPagination.MODE_FULL))

        compact_mode_btn = QPushButton("Compact Mode")
        compact_mode_btn.clicked.connect(
            lambda: self._set_dynamic_mode(FluentPagination.MODE_COMPACT))

        simple_mode_btn = QPushButton("Simple Mode")
        simple_mode_btn.clicked.connect(
            lambda: self._set_dynamic_mode(FluentPagination.MODE_SIMPLE))
    ...
    def _create_simple_pagination_demo(self) -> QWidget:
        ...
        simple_info = QLabel(
            "Simple pagination with just Previous/Next buttons and page display")
        ...
        self.interactive_simple.page_changed.connect(
            self._handle_interactive_simple_change)
        ...
        self.simple_current_spinbox.valueChanged.connect(
            self._update_interactive_simple)
        ...
        self.simple_total_spinbox.valueChanged.connect(
            self._update_interactive_simple)
    ...
    def _create_data_demo(self) -> QWidget:
        ...
        self.table_pagination.page_size_changed.connect(
        ...
        self.search_pagination.page_changed.connect(
            self._update_search_results)
        self.search_pagination.page_size_changed.connect(
            self._update_search_results)
    ...
    def _create_interactive_demo(self) -> QWidget:
        ...
        self.current_page_spinbox.valueChanged.connect(
            self._update_custom_current_page)
        ...
        medium_stress_btn.clicked.connect(
            lambda: self._run_stress_test(100000))
        stress_controls_layout.addWidget(medium_stress_btn)

        large_stress_btn = QPushButton("1M Items")
        large_stress_btn.clicked.connect(
            lambda: self._run_stress_test(1000000))
        ...
        coordination_info = QLabel(
            "Multiple paginations that can be synchronized")
    ...
    def _create_advanced_demo(self) -> QWidget:
        ...
        algorithm_info = QLabel(
            "Demonstrates page button display logic with different total pages")
        ...
        edge_cases_layout.addWidget(
            QLabel("Various edge cases and special scenarios:"))
        ...
        self.metrics_display.setPlainText(
            "Click buttons above to see performance metrics...")
    ...
    def _setup_event_log(self, parent_layout):
        ...
        self.event_log.setPlaceholderText(
            "Pagination events will be logged here...")
    ...
    def _log_page_change(self, source: str, page: int):
        ...
        self.event_log.append(
            f"[{timestamp}] {source}: Page changed to {page}")
    ...
    def _log_page_size_change(self, source: str, size: int):
        ...
        timestamp = datetime.now().strftime("%H:%M:%S")
        self.event_log.append(
            f"[{timestamp}] {source}: Page size changed to {size}")
    ...
    def _update_table_data(self):
        ...
        for row, item in enumerate(page_data):
            ...
            self.data_table.setItem(
                row, 2, QTableWidgetItem(item["department"]))
            self.data_table.setItem(
                row, 3, QTableWidgetItem(f"${item['salary']:,}"))
            self.data_table.setItem(row, 4, QTableWidgetItem(
                f"{item['experience']} years"))
    ...
    def _update_list_data(self):
        ...
        for item in page_data:
            list_item = QListWidgetItem(
                f"{item['name']} - {item['department']} (${item['salary']:,})")
    ...
    def _run_stress_test(self, total_items: int):
        ...
        self._log_event(
            "Stress Test", f"Loaded {total_items:,} items in {duration:.2f}ms")
    ...
    def _sync_paginations(self, page: int):
        ...
        if hasattr(self, 'sync_enabled') and self.sync_enabled:
            ...
            self._log_event("Sync Master", f"Synced to page {page}")

    def _toggle_sync(self, enabled: bool):
        """Toggle pagination synchronization"""
        self.sync_enabled = enabled
        self._log_event(
            "Sync Control", f"Synchronization {'enabled' if enabled else 'disabled'}")
    ...
    def _modify_dynamic_data(self, change: int):
        ...
        self._log_event(
            "Dynamic Data", f"Total changed from {current_total} to {new_total}")
    ...
    def _set_dynamic_data(self, total: int):
        ...
        self._log_event("Dynamic Data", f"Set total to {total}")
    ...
    def _toggle_theme(self, theme_name: str):
        ...
        self._log_event("Theme", f"Changed to {theme_name}")
